...
Node.js Android is an open-source, cross-platform, JavaScript runtime environment. It
...
#### Basic build tools:
```sh
sudo apt-get install -y build-essential git python3 python3-pip llvm clang gcc-multilib g++-multilib
```

#### Install Android NDK r24 for Linux:
Choose a location where you want to install the Android NDK and run:
```sh
git clone https://github.com/johnnkp/Google_Android_NDK.git --depth 1 -j 32
```
It will create a `android-ndk-r24` folder. Save that path for later.

### Building the Android library on Linux:

#### 1) Clone this repo and check out the `master` branch:

```sh
git clone https://github.com/johnnkp/nodejs-android.git --depth 1 -j 32
```
